...
@interface ChoosePhotoVC ()<TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIActionSheetDelegate,UIImagePickerControllerDelegate,UIAlertViewDelegate,UINavigationControllerDelegate>
...
@property (nonatomic, assign) BOOL showTakePhoto;             ///< 在内部显示拍照按钮
...
@property (nonatomic, assign) BOOL allowPickingGif;           ///< 允许选择GIF图片
...
@property (nonatomic, assign) int columnNumberTF;   ///< 每行展示照片张数

@property (nonatomic, assign) BOOL allowCrop;       ///< 单选模式下允许裁剪
@property (nonatomic, assign) BOOL needCircleCrop;  ///< 使用圆形裁剪框
...
- (void)configCollectionView {
    ...
    _collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 10, self.view.tz_width, self.view.tz_height - 10) collectionViewLayout:layout];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == _selectedPhotos.count) {
        ...
        if ([[asset valueForKey:@"filename"] containsString:@"GIF"] && self.allowPickingGif) {
